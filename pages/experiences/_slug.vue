<template>
  ...
  >
    <div class="relative lg:w-1/2 xs:w-full xs:h-84 lg:h-full post-left">
      <img
        :src="require(`~/assets/img/${experience.img}`)"
        :alt="experience.alt"
    ...
    >
      <h1 class="font-bold text-4xl">{{ experience.title }}</h1>
      <p>{{ experience.description }}</p>
      <p class="pb-4">Post last updated: {{ formatDate(experience.updatedAt) }}</p>
      <!-- table of contents -->
      <nav class="pb-6">
        <ul>
          <li
            v-for="link of experience.toc"
            :key="link.id"
            :class="{
      ...
      <author :author="experience.author" />
      <!-- prevNext component -->
